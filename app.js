...
document.addEventListener("DOMContentLoaded", () => {
            ...
            const HIVE_PLANS = {
                'buzz': {
                    ...
                    color: "#28a745",
                    price: "Free",
                    description: "Post 2 free job listing. Access to limited candidate applications (up to 30 resumes). Basic employer dashboard access. Email notifications for job applicants. Standard listing visibility for 7 days. Community support via email."
                },
                'worker': {
                    name: "Worker Plan",
                    limit: 5,
                    ...
                    color: "#007bff",
                    price: "₹1,999 / month",
                    description: "Post up to 5 active jobs. Access to 50 candidate resumes. Basic resume search filters (location, experience). Branded company profile page. Job promotion on HireHive social channels. 15-day visibility on job board. Email & chat support."
                },
                'colony': {
                    name: "Colony Plan",
                    limit: 15,
                    ...
                    color: "#fd7e14",
                    price: "₹4,999 / month",
                    description: "Post up to 15 active jobs. Access to unlimited resume downloads. Advanced candidate filtering (skills, education, salary range). Access to 'Featured Candidates' pool. Company logo on all job posts. Weekly job performance analytics. Priority placement in search results. Dedicated account manager (email only)."
                },
                'queen': {
                    name: "Queen Plan",
                    limit: 30,
                    ...
                    color: "#6f42c1",
                    price: "₹8,999 / month",
                    description: "Post up to 30 active jobs. Access to premium candidate database. AI-powered candidate recommendations. Smart applicant tracking system (ATS) dashboard. Job posts featured on partner platforms (LinkedIn, Indeed sync optional). Video interview scheduling feature. Custom career page integration (for your website). Dedicated account manager (chat & call support)."
                },
                'hive_master': {
            ...
            const plansNavLink = document.getElementById("plansNavLink"); // <--- ADDED PLANS NAV LINK
            ...
            async function updateHeaderUI() {
                ...
                // MODIFIED: Reset state for all header elements, including the Plans link
                [loginBtn, signupBtn, logoutBtn, dashboardLink, adminLink, welcomeMessage].forEach(el => el.classList.add("hidden"));
                if (plansNavLink) plansNavLink.classList.add("hidden"); // <-- Hide plans link by default
                ...
                if (token) {
                    ...
                    } else if (user.role === 'employer' && plansNavLink) { // <--- CONDITIONALLY SHOW PLANS FOR EMPLOYER
            ...
            if (closeApplicantsModalBtn) {
                closeApplicantsModalBtn.onclick = () => {
            ...
            if (document.getElementById("forgotPasswordForm")) {
                document.getElementById("forgotPasswordForm").addEventListener("submit", async(e) => {
                    ...
                    try {
                        const data = await fetchApi('auth/forgot-password', 'POST', {
                            email
                        });
                ...
                document.querySelectorAll('#plans-view .select-plan-btn').forEach(btn => {
                    if (isEmployer && !btn.disabled) {
                        btn.addEventListener('click', () => {
                            // Trigger modal with simulation message
                            showStatusMessage("Subscription Simulation", `You are simulating the purchase of the ${HIVE_PLANS[btn.dataset.planKey].name}. Click 'Select Plan' in the dashboard to confirm the plan change.`, false);
                        });
                    } else if (!user) {
                        btn.addEventListener('click', () => {
                ...
                modalContent.querySelectorAll('.select-plan-btn').forEach(btn => {
                    if (isEmployer && !btn.disabled) {
                        btn.addEventListener('click', async(e) => {
                            ...
                            try {
                                const data = await fetchApi('employer/subscription', 'PUT', {
                                    newPlanKey: planKey
                                });
                                setLocalUser(data.user);
            ...
            document.querySelectorAll(".apply-btn:not([disabled])").forEach((button) => {
                button.onclick = async (e) => {
                    ...
                    try {
                        ...
                        await fetchApi(`seeker/apply/${jobId}`, 'POST', {
                            answers
                        });
